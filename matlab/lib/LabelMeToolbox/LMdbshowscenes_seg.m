function LMdbshowscenes_seg(D, HOMEIMAGES, subimages, showanno, titles)
%
    ...
    for y = Ny-1:-1:0
        for x = 0:Nx-1
            ...
            if nargin>1
                ...
                [~,filename] = fileparts(D(i).annotation.filename);
                filename = fullfile('SUN397_BSR', D(i).annotation.folder, [filename '.mat']);
                try
                    img = load(filename, 'ucm');
                    img = (1-img.ucm)*255;
                catch
                    continue;
                end
