...
opts.sigma = 0 ;
opts.addLcs = false ;
opts.lcsNormalizeComponents = true ;
opts.lcsNormalizeAll = true ;
opts.lcsSquareRoot = false ;
...
for si = 1:numel(opts.scales)
  ...
  % add LCS features
  if opts.addLcs
    lcs = getLCS(imcolor, frames{si}, opts.binSize / opts.scales(si), ...
      opts.lcsNormalizeComponents, opts.lcsNormalizeAll, ...
      opts.lcsSquareRoot) ;

    lcs(:,kill) = 0 ;
    descrs{si} = cat(1, descrs{si}, single(lcs));
    clear lcs;
  end
end
