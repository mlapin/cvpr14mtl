...
*
*   lambda  1-by-1 scalar regularization parameter
*             must be positive
*
* Outputs:
*   A       N-by-T matrix of dual variables
*             type: full float/double matrix (same type as K)
*             note: the D-by-T matrix of classifiers is given by
*               W = X * A;
*
...
#include "mexutils.h"
#include "sdca/svmsolver.h"

void printUsage() {
  mexPrintf("Usage: A = svmsdca(Y,K,lambda);\n"
            "       [A,info] = svmsdca(Y,K,lambda,<options>);\n\n"
            ...
            sdca::SvmSolver<double>::DefaultSeed());
}
...
/* info struct */
template <typename T>
mxArray * createInfoStruct(sdca::SvmSolver<T> &solver) {
  void const *fields [] = {
    "Solver", mxCreateString("SVMSDCA"),
    ...
    "Lambda", mxCreateScalar(static_cast<double>(solver.lambda())),
...
void run(const int nin, const mxArray* in[], const int in_next,
  const int nout, const mxArray *mxY, const mxArray *mxK, const double lambda,
  mxArray **pA, mxArray **pInfo) {
  ...
  sdca::SvmSolver<Q> solver(static_cast<sdca::SdcaSize>(N),
    static_cast<sdca::SdcaSize>(T), Y, static_cast<Q*>(mxGetData(mxK)),
    static_cast<Q*>(mxGetData(mxA)));

  solver.lambda(static_cast<Q>(lambda));
...
void mexFunction(int nout, mxArray* out[], int nin, const mxArray* in[]) {
  ...
  enum {IN_Y = 0, IN_K, IN_LAMBDA, IN_END};
  ...
  try {
    if (mxIsSingle(mxK)) {
      run<float>(nin, in, IN_END, nout, mxY, mxK, lambda, &mxA, &mxInfo);
    } else {
      run<double>(nin, in, IN_END, nout, mxY, mxK, lambda, &mxA, &mxInfo);
    }
  } catch (std::exception &ex) {
      mexErrMsgIdAndTxt(errSolverError, "SVMSDCA: %s", ex.what());
  } catch(...) {
    mexErrMsgIdAndTxt(errSolverError, "SVMSDCA: exception occurred.");
  }
